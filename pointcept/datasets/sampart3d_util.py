...
def sample_surface(mesh, count, face_weight=None, sample_color=False, seed=147):
    ...
    #测试：撤除uv-sampling后，knight分割结果是否变差？（即，衡量uv-sampling对模型分割性能影响）
    if sample_color and hasattr(mesh.visual, "uv") and mesh.visual.uv is not None:
        ...
        print("[Info] mesh.visual.uv is missing — skipping uv-based sampling.")
    ...
    if sample_color:
        if hasattr(mesh.visual, "uv") and mesh.visual.uv is not None:
            ...
            # 没有颜色信息，填充默认中灰色
            colors = np.ones_like(samples) * 127
            print("[Info] 没有颜色信息，默认填充灰色")
