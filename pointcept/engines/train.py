    ...
    def train(self):
        with EventStorage() as self.storage:
            ...
            if self.backbone_weight_path is not None:
                self.logger.info("=> Loading backbone weights ...")
                if os.path.isfile(self.backbone_weight_path):
                    ...
                    for key, value in checkpoint["state_dict"].items():
                        ...
                        key = "backbone." + key
                        if "scale_statistics" in key:
                            continue
                        backbone_weight[key] = value
                    load_info = self.model.load_state_dict(backbone_weight, strict=False)
                    self.logger.info(f"[Backbone] Missing keys: {load_info[0]}")
                else:
                    self.logger.warning(f"No backbone weights found at {self.backbone_weight_path}")

        # Step 2: Load previous training checkpoint (5000.pth)
            if self.cfg.weight is not None:
                self.logger.info("=> Loading fine-tuned MLP weights ...")
                if os.path.isfile(self.cfg.weight):
                    checkpoint = torch.load(
                        self.cfg.weight,
                        map_location=lambda storage, loc: storage.cuda()
                    )
                    mlp_weight = OrderedDict()
                    for key, value in checkpoint["state_dict"].items():
                        if "instance_net" in key or "pos_net" in key:
                            mlp_weight[key] = value
                    load_info = self.model.load_state_dict(mlp_weight, strict=False)
                    self.logger.info(f"[MLP] Missing keys: {load_info[0]}")
                else:
                    self.logger.warning(f"No fine-tuned MLP weights found at {self.cfg.weight}")
#---------------------------------------------------------------------------------------------------------------------
#--------------------------------------------------------------------------------------------------------------------
            for self.epoch in range(self.start_epoch, self.max_epoch):
                self.model.train()
    ...
    def eval(self):
        ...
        for scale in self.val_scales_list:
            ...
            #聚类开始
            self.logger.info(
                f"Clustering on {instance_feat.shape[0]} samples with {instance_feat.shape[1]}-dim features")
            ...
            end_time = time.time()
            self.logger.info(f"HDBSCAN@scale={scale} took {end_time - start_time:.3f} seconds")
            #聚类结束
